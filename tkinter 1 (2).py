...
# --- Custom TextEdit with vertically centered placeholder ---
class CenteredPlaceholderTextEdit(QTextEdit):
    ...
    def paintEvent(self, event):
        ...
        if not self.toPlainText():  # Show placeholder only if empty
            painter = QPainter(self.viewport())
            painter.setPen(QColor("#999999"))
            painter.setFont(self.font())
            rect = self.viewport().rect()
            # Draw placeholder vertically centered, left aligned
            painter.drawText(rect.adjusted(5, 2, 0, 0),
...
# --- Hover Icon Widget (with hover-up animation) ---
class IconWithHoverLabel(QWidget):
    ...
    def enterEvent(self, event):
        ...
        if self._normal_pos is None:
            self._normal_pos = self.button.pos()

        self._hover_anim = QPropertyAnimation(self.button, b"pos")
        self._hover_anim.setDuration(150)
        self._hover_anim.setStartValue(self.button.pos())
        self._hover_anim.setEndValue(self._normal_pos - QPoint(0, 6))
    ...
    def leaveEvent(self, event):
        ...
        if self._normal_pos is not None:
            ...
            self._hover_anim.setDuration(150)
            self._hover_anim.setStartValue(self.button.pos())
            self._hover_anim.setEndValue(self._normal_pos)
            self._hover_anim.setEasingCurve(QEasingCurve.Type.OutQuad)
            self._hover_anim.start()
...
class AnimatedBorderWidget(QWidget):
    def __init__(self):
        ...
        self.text_input = CenteredPlaceholderTextEdit("How can I help you today?")
        ...
        mic_widget = IconWithHoverLabel("🎙️", "Voice", self.empty_voice_function)
    ...
    def adjust_height(self):
        ...
        line_height = self.text_input.fontMetrics().height()
        max_lines = 4
        max_height = (line_height * max_lines) + 40

        new_height = min(max_height, int(doc_height) + 40)
    ...
    def paintEvent(self, event):
        ...
        inner_rect = self.rect().adjusted(self.border_width, self.border_width,
                                          -self.border_width, -self.border_width)
        painter.setPen(Qt.PenStyle.NoPen)
        painter.setBrush(self.inner_bg_color)
        painter.drawRoundedRect(inner_rect, self.radius - self.border_width,
...
# --- Property for rainbow border animation ---
